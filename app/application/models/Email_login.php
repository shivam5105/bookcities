    ...
    public function otp_check()
    {
		...
		if ($query->num_rows() > 0)
		{
			...
			foreach ($records as $key => $record)
			{
				...
				if($otp_expiry <= time())
				{
					return array("response" => false, "msg" => "OTP has expired!", "id" => 0);
				}
				else
				{
					return array("response" => true, "msg" => "OTP successfully varified!", "id" => $record->id);
				}
			}
		}
		else
		{
			return array("response" => false, "msg" => "OTP doesn't exists!", "id" => 0);
		}		
	}
}